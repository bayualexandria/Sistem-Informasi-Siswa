<?php foreach ($siswa as $s) : ?>
    <div class="relative z-10 hidden" id="add<?= $s->id ?>" aria-labelledby="modal-title" role="dialog" aria-modal="true">
        ...
        <div class="fixed inset-0 z-10 overflow-y-auto">
            <div class="flex items-center justify-center min-h-full p-4 text-center ">
                ...
                <div class="relative overflow-hidden text-left transition-all transform bg-white rounded-lg shadow-xl sm:my-8 sm:max-w-lg sm:w-full">
                    <form action="<?= base_url('/siswa/adduser/' . $s->no_induk); ?>" method="post">
                        <div class="px-4 pt-5 pb-4 bg-white sm:p-6 sm:pb-4">
                            ...
                            <div class="flex flex-col justify-start p-4 modal-body gap-y-4">
                                <div class="flex flex-col w-full gap-y-2">
                                    ...
                                    <input type="text" class="w-full px-3 py-2 border rounded-md text-slate-500 focus:outline-none focus:ring-1 bg-slate-300" name="username" value="<?= old('username') ?? $s->no_induk; ?>" placeholder="NIP" readonly />
                                </div>
                        ...
                        <div class="flex flex-col gap-2 px-4 py-3 bg-gray-50 md:flex-row md:justify-end">
                            ...
                            <button type="button" class="px-4 py-2 text-base font-bold text-white transition duration-200 rounded-md bg-slate-500 hover:bg-white hover:text-slate-500 hover:ring-1 hover:ring-slate-300" onclick="showModal('addBtnModal<?= $s->id ?>')">Batal</button>
                        </div>
                    </form>
                </div>
            </div>

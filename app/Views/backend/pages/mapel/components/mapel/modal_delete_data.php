...
<div class="relative z-10 hidden" id="delete<?= $m->id ?>" aria-labelledby="modal-title" role="dialog" aria-modal="true">
      ...
      <div class="relative overflow-hidden text-left transition-all transform bg-white rounded-lg shadow-xl sm:my-8 sm:max-w-lg sm:w-full">
        ...
        <form action="<?= base_url('/mata-pelajaran/' . $m->id); ?>" method="post">
          <div class="px-4 pt-5 pb-4 text-center bg-white text-slate-700">
            ...
            <p>Apakah anda ingin menghapus data mata pelajaran <?= $m->nama_mapel; ?> ?</p>
          </div>
          <div class="flex flex-col gap-2 px-4 py-3 md:justify-center bg-gray-50 md:flex-row">
            ...
            <button type="button" class="px-4 py-2 text-base font-bold text-white transition duration-200 rounded-md bg-sky-500 hover:bg-white hover:text-sky-500 hover:ring-1 hover:ring-sky-300" onclick="showModal('deleteBtnModal<?= $m->id ?>')">Tidak</button>
          </div>
        </form>
      </div>
    </div>

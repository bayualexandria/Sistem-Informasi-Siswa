<?php foreach ($guru as $g) : ?>
    <div class="relative z-10 hidden" id="update<?= $g->id ?>" aria-labelledby="modal-title" role="dialog" aria-modal="true">
        ...
        <div class="fixed inset-0 z-10 overflow-y-auto">
            <div class="flex items-center justify-center min-h-full p-4 text-center ">
                ...
                <div class="relative overflow-hidden text-left transition-all transform bg-white rounded-lg shadow-xl sm:my-8 sm:max-w-lg sm:w-full">
                    <form action="<?= base_url('/guru/updateuser/' . $g->nip); ?>" method="post">
                        ...
                        <div class="flex flex-col px-4 py-3 bg-gray-50 gap-x-2 gap-y-2 md:flex-row md:justify-end">
                            ...
                            <button type="button" class="px-4 py-2 text-base font-bold text-white transition duration-200 rounded-md bg-slate-500 hover:bg-white hover:text-slate-500 hover:ring-1 hover:ring-slate-300" onclick="showModal('updateBtnModal<?= $g->id ?>')">Batal</button>
                        </div>
                    </form>
                </div>
            </div>

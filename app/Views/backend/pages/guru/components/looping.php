        ...
        <tr>
            <td class="px-6 py-[10px] text-sm text-center text-gray-500">
                <div class="w-8 h-8 overflow-hidden rounded-full shadow-md">
                    <img src="<?= base_url('assets/img/profile/guru/' . $g->image_profile); ?>" alt="profile">
                </div>
            </td>
            <td class="px-6 py-[10px]">
                <div class="text-sm text-center text-gray-900">
                    <?= htmlspecialchars_decode($g->nama); ?>
                </div>
            </td>
            <td class="px-6 py-[10px]">
                <div class="text-sm text-center text-gray-500"><?= $g->nip; ?></div>
            </td>
            <td class="px-6 py-[10px] text-sm text-center text-gray-500">
                <?php $time = Time::parse($g->created_at, 'Asia/Jayapura');
                echo $time->toLocalizedString('d MMM yyyy'); ?>
            </td>

            <td class="px-6 py-[10px] text-sm text-center flex text-gray-500 justify-center">

                <?php if ($g->user_id != null) : ?>
                    <button data-bs-toggle="modal" data-bs-target="#update<?= $g->id ?>" id="updateBtnModal<?= $g->id ?>" onclick="showModal('updateBtnModal<?= $g->id ?>')">
                ...
                <?php else : ?>
                    <button type="button" data-bs-toggle="modal" data-bs-target="#add<?= $g->id ?>" id="addBtnModal<?= $g->id ?>" onclick="showModal('addBtnModal<?= $g->id ?>')" onclick="showModal('addBtnModal<?= $g->id ?>')">
            ...
            <td class="px-6 py-[10px]">
                <button type="button" data-bs-toggle="modal" id="editBtnModal<?= $g->id ?>" onclick="showModal('editBtnModal<?= $g->id ?>')" onclick="showModal('editBtnModal<?= $g->id ?>')" data-bs-target="#edit<?= $g->id ?>" class="flex justify-center">
            ...
            <td class="px-6 py-[10px]">
                <button type="button" data-bs-toggle="modal" id="deleteBtnModal<?= $g->id ?>" onclick="showModal('deleteBtnModal<?= $g->id ?>')" onclick="showModal('deleteBtnModal<?= $g->id ?>')" class="flex justify-center" data-bs-toggle="modal" data-bs-target="#delete<?= $g->id ?>">
            ...
            <div class="flex items-center justify-center min-h-full p-4 text-center ">
                ...
                <div class="relative w-full overflow-hidden text-left transition-all transform bg-white rounded-lg shadow-xl md:w-1/4">
                    <div class="flex flex-col items-center justify-center p-5 gap-y-5">
                        ...
                        <p class="text-base font-bold text-center text-slate-500">Data yang anda cari tidak ada</p>

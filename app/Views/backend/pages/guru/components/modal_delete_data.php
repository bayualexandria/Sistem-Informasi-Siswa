<?php foreach ($guru as $g) : ?>
    <div class="relative z-10 hidden" id="delete<?= $g->id ?>" aria-labelledby="modal-title" role="dialog" aria-modal="true">
        ...
        <div class="fixed inset-0 z-10 overflow-y-auto">
            <div class="flex items-center justify-center min-h-full p-4 text-center ">
                ...
                <div class="relative overflow-hidden text-left transition-all transform bg-white rounded-lg shadow-xl sm:my-8 sm:max-w-lg sm:w-full">
                    ...
                    <form action="<?= base_url('/guru/' . $g->nip); ?>" method="post">
                        <div class="px-4 pt-5 pb-4 text-center bg-white text-slate-700">

                            <?= csrf_field(); ?>
                            <input type="hidden" name="_method" value="DELETE">
                            <p>Apakah anda ingin menghapus data Guru dengan nama <?= $g->nama; ?> ?</p>
                        </div>
                        <div class="flex flex-col gap-2 px-4 py-3 md:justify-center bg-gray-50 md:flex-row">
                            ...
                            <button type="button" class="px-4 py-2 text-base font-bold text-white transition duration-200 rounded-md bg-sky-500 hover:bg-white hover:text-sky-500 hover:ring-1 hover:ring-sky-300" onclick="showModal('deleteBtnModal<?= $g->id ?>')">Tidak</button>
                        </div>
                    </form>
                </div>
            </div>

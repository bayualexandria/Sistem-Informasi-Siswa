<?php foreach ($guru as $g) : ?>
    <div class="relative z-10 hidden" id="edit<?= $g->id ?>" aria-labelledby="modal-title" role="dialog" aria-modal="true">
            ...
            <div class="flex items-center justify-center min-h-full p-4 text-center ">
                <div class="relative overflow-hidden text-left transition-all transform bg-white rounded-lg shadow-xl sm:my-8 sm:max-w-lg sm:w-full">
                    <div class="px-5 pt-5">
                        ...
                        <h1 class="text-base font-bold text-slate-500">Update Data Guru</h1>
                    </div>
                    <form action="<?= base_url('/guru/' . $g->nip); ?>" method="post" enctype="multipart/form-data">
                        <div class="px-4 pt-5 pb-4 bg-white sm:p-6 sm:pb-4">
                            ...
                            <div class="flex flex-col p-4 modal-body gap-y-2">
                                <div class="flex flex-col w-full gap-y-2">
                                    <label for="nip" class="text-base font-normal text-slate-600">NIP</label>
                                    <input type="text" class="w-full px-3 py-1 text-slate-500 border rounded-md bg-slate-300 focus:outline-none focus:ring-1 <?= $validation->hasError('nip') ? 'border-red-500 focus:ring-red-500' : 'border-sky-500 focus:ring-slate-500'; ?>" name="nip" value="<?= $g->nip; ?>" readonly />
                                    <span class="flex">
                                    ...
                                    <div class="flex flex-col w-full gap-2 ">
                                        ...
                                        <div class="flex flex-col justify-start md:flex-row gap-x-2">
                                            <div class="flex flex-row gap-x-1">
                                                <input class="w-3.5 h-3.5 mt-1 mr-2 transition duration-200 bg-white bg-center border rounded-full appearance-none cursor-pointer checked:bg-primary checked:border-primary focus:outline-none align-center border-primary" type="radio" name="jenis_kelamin" value="Laki-laki" <?= $g->jenis_kelamin === 'Laki-laki' ? 'checked' : ''; ?>>
                                                <label class="inline-block text-gray-800 form-check-label" for="Laki-laki">Laki-laki</label>
                                            </div>
                                            <div class="flex flex-row gap-x-1">
                                                <input class="w-3.5 h-3.5 mt-1 mr-2 transition duration-200 bg-white bg-center border rounded-full appearance-none cursor-pointer checked:bg-primary checked:border-primary focus:outline-none align-center border-primary" type="radio" name="jenis_kelamin" <?= $g->jenis_kelamin === 'Perempuan' ? 'checked' : ''; ?> value="Perempuan">
                                ...
                                <div class="grid grid-cols-2 gap-x-2">
                                    ...
                                    <div class="pt-2">
                                        <?php if ($g->image_profile) : ?>
                                            <img src=" <?= base_url('assets/img/profile/guru/' . $g->image_profile); ?>" alt="cover" class="rounded-md img-preview" id="img-preview<?= $g->id ?>">
                                        <?php else : ?>
                                            <img src="" alt="cover" class="hidden rounded-md img-preview" id="img-preview<?= $g->id ?>">
                                        <?php endif; ?>
                        ...
                        <div class="px-4 py-3 bg-gray-50 sm:px-6 sm:flex sm:flex-row-reverse gap-x-2">
                            ...
                            <button type="button" class="px-2 py-1 text-base font-bold text-white transition duration-200 rounded-md bg-slate-500 hover:bg-white hover:text-slate-500 hover:ring-1 hover:ring-slate-300" onclick="showModal('editBtnModal<?= $g->id ?>')">Batal</button>
                        </div>
                    </form>
                </div>
            </div>
